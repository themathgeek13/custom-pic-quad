...
void	UARTInitRX(int IL, int BaudRate)
	...
	//=========================================================
	UMODE			= 0;		// Disable UART for configuration
	...
	// later enabling UART module we need to enable module in
	// PMDx
	//---------------------------------------------------------
	UPMD	= 0;		// Enable UART module
	//---------------------------------------------------------
	...
	//---------------------------------------------------------	
	// As we use UART ONLY to receive data, to "save" MCU pins 
	// we may leave TX unmapped.
	//---------------------------------------------------------
	// Configure Receiver's pins...
	//---------------------------------------------------------
	U_RPS_RX 	= RXpin; 		// (IN)  RX mapped to Receiver pin
	//----------------------
	RTSpin 		= U_RPS_RTSid; 	// (OUT) RTS pin mapped to respective
								// UART Module RTS function
	//-------------------------------------------------------------------
	...
	bit 0 STSEL: Stop Selection bit
			...
			0 = 1 Stop bit	
	*/	
	UEN	= 0b01;		// UxTX, UxRX and UxRTS pins are enabled and
					// used; UxCTS pin controlled by port latches.
				 	// ----------------------------------------
	//---------------------------------------------------------
	...
	*/
	...
	switch (BaudRate)
		{
		//--------------------------------------------------------------
		// The UBRG values correspond to following speeds:
		//		BRG	Baud Rate
		//		0	2,500,000
		//		1	1,250,000
		//		2	833,333
	...
	URXIP	= _UART_IL_RX; 	// Set UART RX interrupt priority
	UREIP	= _UART_IL_RX; 	// Set UART RX Error interrupt priority 									
	//---------------------------------------------------------
