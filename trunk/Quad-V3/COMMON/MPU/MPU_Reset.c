				 ...
				 MPU_FS_SEL GS, MPU_AFS_SEL AS)
{
	...
	//---------------------------------------------------------
	RC	= MPUSetByte(MPU6050_PWR_MGMT_1, 0x80);
	...
	do 
		...
		//--------------------------------
		RC = MPUGetByte(MPU6050_PWR_MGMT_1, &RegValue);
		if (RC) return RC;	// Failure...
		//--------------------------------
	...
	//---------------------------------------------------------
	RC	= MPUSetByte(MPU6050_PWR_MGMT_1, 0x01);
	if (RC) return RC;	// Failure...
	// Verify PWR_MGMT_1
	RC = MPUGetByte(MPU6050_PWR_MGMT_1, &RegValue);
	if (RC) return RC;	// Failure...
	...
	//---------------------------------------------------------
	RC	= MPUSetByte(MPU6050_PWR_MGMT_2, 0x00);
	if (RC) return RC;	// Failure...
	// Verify PWR_MGMT_2
	RC = MPUGetByte(MPU6050_PWR_MGMT_2, &RegValue);
	if (RC) return RC;	// Failure...
	...
	RC	= MPUSetByte(MPU6050_CONFIG, DLPF);
	if (RC) return RC;	// Failure...
	// Verify written value
	RC = MPUGetByte(MPU6050_CONFIG, &RegValue);
	if (RC) return RC;	// Failure...
	...
	// ( 1 <= DLPF <= 6)
	RC	= MPUSetByte(MPU6050_SMPLRT_DIV, RateDiv);
	if (RC) return RC;	// Failure...
	// Verify written value
	RC = MPUGetByte(MPU6050_SMPLRT_DIV, &RegValue);
	if (RC) return RC;	// Failure...
	...
	switch (GS)
		{
		case 	MPU_GYRO_250ds:
			_MPU_Gyro_Sensitivity = 0.00013323124;
			RegValue = 0x00;
			break;
		case 	MPU_GYRO_500ds:
			_MPU_Gyro_Sensitivity = 0.00026646248;
			RegValue = 0x01;
			break;
		case 	MPU_GYRO_1000ds:
			_MPU_Gyro_Sensitivity = 0.00053211257;
			RegValue = 0x02;
			break;
		case 	MPU_GYRO_2000ds:
		default:
			_MPU_Gyro_Sensitivity = 0.0010642251;
			RegValue = 0x03;
			break;
		}
	RegValue *= 8;	// Place sensitivity bits into proper position
	RC	= MPUSetByte(MPU6050_GYRO_CONFIG, RegValue);
	if (RC) return RC;	// Failure...
	//=========================================================
	// ACC configuration:
	// XA_ST (selft-test)		0
	...
	switch (AS)
		{
		case 	MPU_ACC_2g:
			_MPU_Accl_Sensitivity = 0.000061035156;
			RegValue = 0x00;
			break;
		case 	MPU_ACC_4g:
			_MPU_Accl_Sensitivity = 0.00012207031;
			RegValue = 0x01;
			break;
		case 	MPU_ACC_8g:
			_MPU_Accl_Sensitivity = 0.00024414062;
			RegValue = 0x02;
			break;
		case 	MPU_ACC_16g:
		default:
			_MPU_Accl_Sensitivity = 0.00048828125;
			RegValue = 0x03;
			break;
		}
	RegValue *= 8;	// Place sensitivity bits into proper position
	RC	= MPUSetByte(MPU6050_ACCEL_CONFIG, RegValue);
	if (RC) return RC;	// Failure...
	//---------------------------------------------------------
	...
	//---------------------------------------------------------
	RC	= MPUSetByte(MPU6050_USER_CTRL, 0x01);
	if (RC) return RC;	// Failure...
	...
	do
		...
		TMRDelayTicks(1);	// Short Delay
		//--------------------------------
		RC = MPUGetByte(MPU6050_USER_CTRL, &RegValue);
		if (RC) return RC;	// Failure...
		//--------------------------------
		if (TMRCheckAlarm(Alarm))	// Time expired without MPU completing RESET
			return MPU_FAIL;
	...
	// CLKOUT diabled		(0)
	//---------------------------------------------
	RC	= MPUSetByte(MPU6050_INT_PIN_CFG, 0x10);
	if (RC) return RC;	// Failure...
	//=========================================================
	// Enable interrupts: Only DATA_RDY_EN
	RC	= MPUSetByte(MPU6050_INT_ENABLE, 0x01);
	if (RC) return RC;	// Failure...
	//=========================================================
	//Clear READY status (if any) by reading STATUS register
	RC	= MPUGetByte(MPU6050_INT_STATUS, &RegValue);
	if (RC) return RC;	// Failure...
	...
	//---------------------------------------------------------
	Alarm	= TMRSetAlarm(2000);	//Set Alarm for 2 seconds.
	//-----------------------------
	do
		{
